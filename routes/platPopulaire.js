...
//import model
const platPopulaire = require('../models/platPopulaire.model');

router.get('/', async function(req, res) {
    try {
        const restaurant = await platPopulaire.find().populate({
            path: 'food',
            populate: {
                path: 'restaurant',
                model: 'Restaurant',
...
router.get('/:platId', adminGuard, async function(req, res) {
    try {
        const plat = await platPopulaire.findById(req.params.platId)
...
router.post('/', adminGuard, async function(req, res, next) {
    ...
    try {
        ...
        for (let i = 0; i < data.food.length; i++) {
            ...
            await platPopulaire.create({
                ...data,
                food: data.food[i],
                priority: await getNextSequenceValue('platPopulairePriority'),
            });
...
router.put('/:platId', adminGuard, async function(req, res) {
    ...
    try {
        await platPopulaire.updateOne({ _id: platId }, data);
...
router.delete('/:platId', adminGuard, async function(req, res) {
    try {
        const ressource = await platPopulaire.findOneAndRemove({
            _id: req.params.platId,
        });

        if (
            ressource.priority ===
            (await getCurrentSequenceValue('platPopulairePriority'))
        )
            await decrementSequenceValue('platPopulairePriority');

        const result = await platPopulaire.deleteOne({ _id: req.params.platId });

        if (!(await platPopulaire.count()))
            resetSequenceValue('platPopulairePriority');
